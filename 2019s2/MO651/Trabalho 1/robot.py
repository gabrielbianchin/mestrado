...
class Robot():
	...
	def get_current_position(self):
		...
		#return position
		erro, posicao = vrep.simxGetObjectPosition(self.clientID, self.robot_handle, -1, vrep.simx_opmode_oneshot_wait)
		self.matrizTranslacaoGT = [[1, 0, posicao[0]], [0, 1, posicao[1]], [0, 0, 1]]
		return posicao
	...
	def get_right_angle(self):
		...
		"""
		res, angle = vrep.simxGetJointPosition(self.clientID, self.motors_handle["right"], vrep.simx_opmode_streaming)
		while(res != vrep.simx_return_ok):
			res, angle = vrep.simxGetJointPosition(self.clientID, self.motors_handle["right"], vrep.simx_opmode_streaming)
		return angle
	...
	def get_left_angle(self):
		"""
			...
			Returns:
				angle: Angle of the right wheel.
		"""
		res, angle = vrep.simxGetJointPosition(self.clientID, self.motors_handle["left"], vrep.simx_opmode_streaming)
		while(res != vrep.simx_return_ok):
			res, angle = vrep.simxGetJointPosition(self.clientID, self.motors_handle["left"], vrep.simx_opmode_streaming)
		return angle

	def odometry(self, Wr, Wl, posX, posY, posO, deltaT):
		"""
		...
		"""
		w = (self.WHEEL_RADIUS * (Wr - Wl)) / (2 * self.L)
		deltaO = w * deltaT
		deltaS = ((self.WHEEL_RADIUS * (Wr - Wl)) / 2) * deltaT

		posX += (deltaS * math.cos(self.ORIENTATION + (deltaO / 2)))
		posY += (deltaS * math.sin(self.ORIENTATION + (deltaO / 2)))
		posO += deltaO

		return posX, posY, posO
